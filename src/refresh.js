...
let ampchart = new Chart(
	document.getElementById('ampchart'),
	{
		type: 'line',
		data: {
		labels: batteryvoltdata.map(row => row.seconds),
...
async function download() {
	await invoke("download", { fileloc: 'batteryvolt.json', contents: JSON.stringify(batteryvoltdata), homedir: 'sherlock' });
	await invoke("download", { fileloc: 'batteryamp.json', contents: JSON.stringify(batteryampdata), homedir: 'sherlock' });
	await invoke("download", { fileloc: 'batterytemp.json', contents: JSON.stringify(tempdata), homedir: 'sherlock' });
}
...
function refresh() {
	if (start_status == true) {
		axios.get('http://localhost:8081/volatge/battery')
		.then(function (response) {
			...
			batteryvoltdata.push({ seconds: elapsedSeconds, voltage: Number(data)});
		...
		axios.get('http://localhost:8081/amperage/battery')
		.then(function (response) {
			console.log(response);
			let data = response.data;
			let currentSeconds = Date.now() / 1000;
			...
			document.getElementById("battcurrenttext").innerText = "Battery Current: " + data + " A";
		})
		.catch(function (error) {
			console.log(error);
		})
		...
		axios.get('http://localhost:8081/temperature')
		.then(function (response) {
			console.log(response);
			let data = response.data;
			let currentSeconds = Date.now() / 1000;
			...
			document.getElementById("batttemptext").innerText = "Battery Temperature: " + data + " C";
		})
		.catch(function (error) {
			console.log(error);
		})
